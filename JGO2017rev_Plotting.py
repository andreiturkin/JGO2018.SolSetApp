...
from JGO2017rev_jsonSaving import saveDataToFile
...
########################################################################################
    ...
    def drawCircle(self, center, radius):
        self.__ax.add_patch(patches.Circle(center, radius, fill=False, lw=0.1, ls='dashed', color='black'))

    def drawArc(self, center, radius, psi):
        self.__ax.add_patch(patches.Arc(center, 2*radius, 2*radius, 0.0, psi[0]*180.0/np.pi, psi[1]*180.0/np.pi, color='orange', lw=0.1))
    ...
    def drawLine(self, center, radius, psi):
        for angle in psi:
            ...
            self.__ax.plot([x1, x2], [y1, y2], color='orange', lw=0.1)
